...
private final String vertexShader = "attribute vec2 a_position;\n"
    ...
    private final String alphaShader2 = "#extension GL_OES_EGL_image_external : require\n"
            ...
            + "uniform samplerExternalOES sTexture;\n"
            + "void main() {\n"
            +"if(texture2D(sTexture, v_texcoord).rgb=0) \n"
            + "  gl_FragColor = vec4(texture2D(sTexture, v_texcoord).rgb, texture2D(sTexture, v_texcoord + vec2(-0.5, 0)).r);\n"
            + "}\n";
    ...
    public GiftRenderer(AlphaModel alphaModel) {
        this.alphaModel=alphaModel;
        float[] vertices=this.alphaModel==AlphaModel.LEFT_TO_ALPHA_RIGHT?triangleVerticesData:triangleVerticesData1;
...
//        GLES20.glEnable(GL_POLYGON_SMOOTH_HINT);
    ...
    public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
        String shader=alphaModel==AlphaModel.LEFT_TO_ALPHA_RIGHT?alphaShader:alphaShader1;
